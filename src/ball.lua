...
function Ball:draw()
    ...
    if self.isPlayer then
        ...
        if rolling then
            ...
            -- Ears with idle wiggle animation
            local earWiggle = math.sin(t * 1.8) * r * 0.04
            local earTilt   = math.sin(t * 1.8) * 0.08   -- slight rotate
            love.graphics.setColor(shellDark)
            love.graphics.push()
            love.graphics.translate(-r*0.32, -r*1.18)
            love.graphics.rotate(-earTilt)
            love.graphics.ellipse("fill", 0, earWiggle, r*0.14, r*0.28)
            love.graphics.pop()
            love.graphics.push()
            love.graphics.translate(r*0.32, -r*1.18)
            love.graphics.rotate(earTilt)
            ...
            love.graphics.translate(-r*0.32, -r*1.18)
            love.graphics.rotate(-earTilt)
            love.graphics.ellipse("fill", 0, earWiggle, r*0.07, r*0.17)
            love.graphics.pop()
            love.graphics.push()
            love.graphics.translate(r*0.32, -r*1.18)
            love.graphics.rotate(earTilt)
