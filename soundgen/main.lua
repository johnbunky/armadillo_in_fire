...
local function writeWav(soundData)
    ...
    local byteRate   = sampleRate * channels * bitDepth / 8
    local dataSize   = samples * channels * bitDepth / 8

    -- Build WAV header (44 bytes) + raw PCM data
    ...
    local header =
        "RIFF" .. i32(36 + dataSize) .. "WAVE" ..
        "fmt " .. i32(16) .. i16(1) ..          -- PCM
        i16(channels) .. i32(sampleRate) ..
        i32(byteRate) .. i16(channels * bitDepth / 8) ..
        i16(bitDepth) ..
        "data" .. i32(dataSize)

    -- Write samples as 16-bit signed PCM
    local pcm = {}
    for i = 0, samples - 1 do
        local s = math.max(-1, math.min(1, soundData:getSample(i)))
        local v = math.floor(s * 32767)
        if v < 0 then v = v + 65536 end
        pcm[#pcm+1] = string.char(v % 256, math.floor(v / 256) % 256)
    end

    return header .. table.concat(pcm)
end

local function save(name, soundData)
    love.filesystem.createDirectory("assets/sounds")
    local path    = "assets/sounds/" .. name
    local ok, err = love.filesystem.write(path, writeWav(soundData))
    if ok then
        print("Saved: " .. path)
    else
        print("ERROR: " .. path .. " - " .. tostring(err))
    end
end
